...
Exposes the Swarms API as MCP tools so any MCP client (Claude Code, Claude
Desktop, Cursor) can run swarms directly. Run it with `python swarms_api_mcp_server.py`.

Run:
    python swarms_api_mcp_server.py
...
class AgentSpec(BaseModel):
    ...
    model_name: Optional[str] = Field(
        default="gpt-4.1",
        description="The name of the AI model that the agent will utilize for processing tasks and generating outputs. For example: gpt-4o, gpt-4.1, openai/o3-mini",
    )
